...
<br>
<strong>Demo:</strong> https://youtu.be/KWaFtcJf4lA
...
На екранот покрај 5те животи(срцињата) и левелот(катот) на играчот кои се наоѓаат во десниот горен агол во левиот горен агол ги има и опцијата mute/unmute, највисокиот постигнат резултат и тековниот резултат на играчот.

На секој 10 собрани компири (секој следен левел/кат) се зголемува брзината на паѓање на компирите.

На секој промашен компир се менува изразот на ликот на Цацко и паднатиот компир станува смачкан. <br>
...
Овде bool променливата `dir` ни служи како маркер за промена на сликата кога на Цацко ќе му падден компир. Имплементирана е на следен начин:
...
#### 5 Hearts:
Играчот има 5 животи или 5 можности да промаши компир. При градење во формата се ставени 5 полни срциња и со секое промашување почнувајќи од лево кон десно се менува сликата во црно-бела:
```c#
        void hearts()
        {
            if (missed == 1)
...
```
 Со тоа што има мало поместување вредноста на координатата `y` зависно дали Цацко ќе го фати компирот или не со цел секој пат да изминуваат исто растојание при паѓање.
